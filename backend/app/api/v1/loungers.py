...
async def get_loungers(db: AsyncSession = Depends(get_db), _user: User = Depends(get_current_active_user)):
    result = await db.execute(select(Lounger))
    return result.scalars().all()

@router.post("/", response_model=LoungerSchema)
async def create_lounger(lounger_in: LoungerCreate, db: AsyncSession = Depends(get_db), _user: User = Depends(get_current_active_user)):
...
async def delete_lounger(lounger_id: int, db: AsyncSession = Depends(get_db), _user: User = Depends(get_current_active_user)):
...
async def update_lounger_status(lounger_id: int, status: LoungerStatus, db: AsyncSession = Depends(get_db), _user: User = Depends(get_current_active_user)):
    result = await db.execute(select(Lounger).filter(Lounger.id == lounger_id))
    lounger = result.scalars().first()
    if not lounger:
        raise HTTPException(status_code=404, detail="Топчан не найден")
